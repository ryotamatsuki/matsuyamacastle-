...
test('walk starts and resets at the Honmaru plaza cinematic viewpoint',async({page},info)=>{
 ...
 expect(Math.hypot(moved.x-before.x,moved.z-before.z)).toBeGreaterThan(.5);expect(moved.yaw).not.toBeCloseTo(before.yaw,5);
...
test('continuous production Walker route from plaza to top and third-floor wall constraint',async({page})=>{
 ...
 const result=await page.evaluate(()=>{
  ...
  const plazaToCourtyard=[[-2,60],[-5.05,55],[-5.05,44.55],[-5.05,41.5],[-5.05,38.5],[-5.05,35.5],[-5.05,32.5],[-5.05,30.05],[-4.95,28.45],[1.6,28.45],...nine.reverse(),[1.6,22.55],[3.7,21.75],[3.7,18.45],[-1.9,18.45],[-1.9,16.65],[-1.9,14.9],[0,13.1]];
