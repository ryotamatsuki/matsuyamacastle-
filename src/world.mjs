...
export function blocked(x,y,z,r=.23){
 ...
 // Low C sidewalls follow the same sloping lower/upper elevations as the rendered mesh.
 for(const b of approachRetainingWalls){
  if(l.x<b.x0-r||l.x>b.x1+r||l.z<b.z0-r||l.z>b.z1+r)continue;
  const t=Math.max(0,Math.min(1,(l.z-b.z0)/(b.z1-b.z0)));
  const bottom=b.bottom0+(b.bottom1-b.bottom0)*t,top=b.top0+(b.top1-b.top0)*t;
  if(y+1.55>bottom&&y+.08<top)return true;
 }
