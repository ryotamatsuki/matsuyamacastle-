...
import {toLocal,toWorld,assumptions} from './data/unifiedLayout.mjs';
...
document.querySelectorAll<HTMLButtonElement>('[data-panel]').forEach(b=>b.onclick=()=>{
 ...
 if(kind==='help')openPanel('操作方法','<p>PC：WASDで移動、マウスで見回す、Shiftで早歩き。Escで視点操作を解除します。階段はそのまま歩いて上れます。</p><p>iPhone / iPad：左スティックで移動、画面右側をドラッグして視点を操作します。同時に操作できます。横画面を推奨します。</p><p>現行推定経路：内庭 → 同じPLATEAU天守の入口 → 穴蔵左側 → 1階右側 → 2階左側 → 最上階。階段位置はC — gameplay interpolationで、実在位置を証明するものではありません。</p>');
...
Promise.all([aerialReady,loader.loadAsync(base+'models/matsuyama_keep.glb'),fetch(base+'data/source_manifest.json').then(r=>{if(!r.ok)throw Error('source_manifest '+r.status);return r.json();})]).then(([,gltf,sources])=>{
 ...
 walkScene=gltf.scene;scene.add(gltf.scene);loaded=true;$('#start').removeAttribute('disabled');$('#start').textContent='散歩をはじめる';$('#load-status').textContent='PLATEAU連立天守群 · 内庭 → 穴蔵 → 最上階';
...
function frame(now:number){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;
 ...
 const i=floorIndex();$('#location').textContent=i<0?('天守丸・内庭'):D.floors[i].name;
 $('#route').textContent=i<0?'内庭から、目の前の天守入口へ':i===0?'左の階段へ · C推定経路':i===1?'右奥の階段から2階へ · C推定経路':i===2?'左手前の階段から3階へ · C推定経路':'最上階 · Bは開口関係、座標はC';
...
if(import.meta.env.VITE_TEST==='1')(window as any).__walkTest={look:(y:number,p:number)=>{yaw=y;pitch=p;},step:(x:number,z:number,dt:number)=>player.step(x,z,dt),reset:()=>player.reset(),set:(x:number,y:number,z:number)=>{player.x=x;player.y=y;player.z=z;},start:()=>{active=true;$('#welcome').hidden=true;$('#hud').hidden=false;$('#touch').hidden=!touchDevice;},world:{floors,walls,ramps,start:D.start,levels:D.floors},toWorld};
