...
let surveyMode=false,surveyScene:T.Group|null=null,walkScene:T.Group|null=null;
const surveyGround=new T.Mesh(new T.CircleGeometry(75,64),new T.MeshStandardMaterial({color:0xb5ae92,roughness:1}));surveyGround.rotation.x=-Math.PI/2;surveyGround.position.y=-.1;surveyGround.receiveShadow=true;surveyGround.visible=false;scene.add(surveyGround);
function setSurvey(on:boolean){
 surveyMode=on;orbit.enabled=on;if(surveyScene)surveyScene.visible=on;if(walkScene)walkScene.visible=!on;landscape.visible=!on;surveyGround.visible=on;
 $('#exterior-view').textContent=on?'散歩用の復元外観へ戻る':'PLATEAUの外観を見る';
 $('#load-status').textContent=on?'公式LOD2形状 · 色・窓・瓦の細部は推定補完 · ドラッグで回転':'本丸 → 穴蔵 → 1階 → 2階 → 3階';
 if(on){camera.position.set(68,45,75);orbit.target.set(0,12,0);orbit.update();}
 Object.assign(sun.shadow.camera,{left:on?-48:-25,right:on?48:25,top:on?48:25,bottom:on?-48:-25,far:on?180:100});sun.shadow.camera.updateProjectionMatrix();
}
...
Promise.all([loader.loadAsync(base+'models/matsuyama_keep.glb'),fetch(base+'data/source_manifest.json').then(r=>{if(!r.ok)throw Error('source_manifest '+r.status);return r.json();})]).then(([gltf,sources])=>{
 ...
 walkScene=gltf.scene;scene.add(gltf.scene);loaded=true;$('#start').removeAttribute('disabled');$('#start').textContent='散歩をはじめる';$('#load-status').textContent='本丸 → 穴蔵 → 1階 → 2階 → 3階';
}).catch(e=>{console.error(e);$('#load-status').textContent='読み込みに失敗しました。通信を確認して再読み込みしてください。';});
// Load the independently attributed exterior only on request, keeping mobile startup light.
let surveyLoading=false;
$('#exterior-view').removeAttribute('disabled');
$('#exterior-view').onclick=async()=>{
 if(surveyScene){setSurvey(!surveyMode);return;}
 if(surveyLoading)return;surveyLoading=true;$('#exterior-view').textContent='LOD2外観を読み込み中…';
 try{
  const gltf=await loader.loadAsync(base+'models/matsuyama_plateau_lod2.glb');surveyScene=gltf.scene;
  surveyScene.traverse(o=>{if(o instanceof T.Mesh){o.castShadow=true;o.receiveShadow=true;const m=o.material as T.MeshStandardMaterial;for(const tex of [m.map,m.normalMap,m.roughnessMap])if(tex)tex.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());}});
  scene.add(surveyScene);setSurvey(true);
 }catch(e){console.error(e);$('#exterior-view').textContent='再試行：PLATEAU外観';}finally{surveyLoading=false;}
};
...
function frame(now:number){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;
 ...
 }else if(surveyMode){orbit.update();}else if(!$('#welcome').hidden&&!panel.open){camera.position.set(25,14,36);camera.lookAt(0,6,0);}
...
(window as any).__castle={get ready(){return loaded},get state(){return {x:player.x,y:player.y,z:player.z,yaw,pitch,active,fps,surveyMode,surveyReady:!!surveyScene,meshes:renderer.info.render.calls}},get floor(){return floorIndex()}};
